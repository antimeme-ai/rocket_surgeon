...
Prerequisites: Rust 1.88+ (edition 2024), [`uv`](https://docs.astral.sh/uv/), and [`lefthook`](https://lefthook.dev/installation/). Everything else (Python interpreter included) is provisioned by `uv` into the project venv. `lefthook install` is wired up by the bootstrap, so git hooks run automatically on commit and push.
