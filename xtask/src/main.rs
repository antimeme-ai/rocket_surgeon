...
fn main() -> Result<()> {
    ...
    match cmd {
        ...
        Xtask::Setup => setup()?,
        Xtask::Watch => watch()?,
        Xtask::TestWatch { pattern } => test_watch(pattern.as_deref())?,
        Xtask::WatchOnce => watch_once()?,
        Xtask::TestWatchOnce { pattern } => test_watch_once(pattern.as_deref())?,
    }
    Ok(())
}
...
fn test_watch(pattern: Option<&str>) -> Result<()> {
    ensure_cargo_watch()?;
        ...
        .filter(|path| {
            path.is_file()
                ...
                && path.file_name().and_then(|n| n.to_str()).is_some_and(|n| {
                    n.starts_with("test_e2e_") && pattern.is_none_or(|p| n.contains(p))
                })
        })
        .collect();
    scripts.sort();
...
fn test_watch_once(pattern: Option<&str>) -> Result<()> {
    let stamp = now_stamp();
    let label = pattern.unwrap_or("<all>");
    ...
    let result = (|| -> Result<()> {
        ...
        let scripts = e2e_scripts_matching(pattern)?;
        if scripts.is_empty() {
